...
void ClareEvo::setupEvo(Logging &Log, Stream &serial) {
    channel = &serial;
...
void ClareEvo::readState(float x1, float x2, float x3, float x4) {
    ...
    uint8_t indexx;// The variable "indexx" will contain the number of actual bytes in the frame to treat in the main loop
