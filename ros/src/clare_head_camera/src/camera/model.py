...
# For the intel face-detection-* models
class IntelFaceDetection(Model):
    def __init__(self, ie, model_path, input_transform, threshold=0.5):
        super().__init__(ie, model_path, input_transform)
        ...
        assert len(self.net.outputs) == 1, "Expected 1 output blobs"
        self.bboxes_blob_name = self._parse_outputs()
    ...
    def _parse_outputs(self):
        ...
        # Assumes only one output
        for name, layer in self.net.outputs.items():
            bboxes_blob_name = name

        return bboxes_blob_name
    ...
    def postprocess(self, outputs, meta):
        ...
        boxes = outputs[self.bboxes_blob_name][0][0]
        scores = boxes[:,2]

        mask = scores > self.confidence_threshold
        filtered_boxes, filtered_score = boxes[mask, 3:], scores[mask]
